<script lang="ts">
         ...
         *
         * Soporta ?codigo=XXXXXXXXXX: el email oficial de la boleta enlaza
         * aquí con el código prellenado para que la consulta sea un clic.
         */
        ...
        async function consultar(normalizado: string): Promise<void> {
                isConsultando = true;
                errorConsulta = '';
                notFound = false;
                ticket = null;
        ...
        async function handleConsulta(e: SubmitEvent): Promise<void> {
                ...
                await consultar(normalizado);
        }

        /** Deep-link del email oficial: /boletas?codigo=... consulta sola al abrir. */
        $effect(() => {
                if (deepLinkAplicado) return;
                const deepLink = (page.url.searchParams.get('codigo') ?? '').trim().toUpperCase();
                if (!deepLink) return;
                deepLinkAplicado = true;
                codigo = deepLink;
                if (CODIGO_PATTERN.test(deepLink)) {
                        void consultar(deepLink);
                } else {
                        errorConsulta = 'El código tiene 10 caracteres (letras y números), sin espacios. Ej: A1B2C3D4E5.';
                }
        });

        /** Imprime solo la tarjeta de la boleta (los estilos @media print lo formatean). */
        function imprimirBoleta(): void {
                window.print();
        }
                        ...
                        {#if ticket}
                                <article class="boletas-resultado" aria-live="polite">
                                        ...
                                        <div class="boletas-acciones">
                                                <button type="button" class="btn boletas-print-btn" onclick={imprimirBoleta}>
                                                        <i class="fa-solid fa-print"></i> Imprimir / Guardar PDF
                                                </button>
                                                <a
                                                        type="button"
                                                        class="btn boletas-wa-btn"
                                                        href="https://wa.me/?text={encodeURIComponent(`¡Compré mi boleta para ${ticket.evento_titulo} de FUNCREES! Mi número: ${ticket.numero_ticket}. Consulta la tuya en https://funcreescolombia.org/boletas`)}"
                                                        target="_blank"
...
<style>
        ...
        /* Acciones del resultado: imprimir y compartir */
        .boletas-acciones {
                display: flex;
                gap: 0.75rem;
                flex-wrap: wrap;
        ...
        .boletas-print-btn:hover,
        .boletas-print-btn:focus-visible {
                background: var(--primary-dark, var(--primary));
                transform: translateY(-2px);
                box-shadow: 0 6px 16px var(--primary-trans, rgba(0, 0, 0, 0.15));
        }
        ...
        .boletas-wa-btn:hover,
        .boletas-wa-btn:focus-visible {
                background: #25d366;
                color: #fff;
                transform: translateY(-2px);
        ...
        /* Impresión: solo la tarjeta de la boleta, en blanco y negro, con
           URLs visibles. Sirve para "Guardar como PDF" desde el navegador. */
        @media print {
                :global(body *) {
                        visibility: hidden;
                }
