<script>
  ...
  import { sendContactMessage } from '$lib/api/client.js';
  import { toast } from '$lib/stores/toast.svelte.js';

  let nombre = $state('');
  let email = $state('');
  let telefono = $state('');
  let asunto = $state('Consulta General');
  let mensaje = $state('');
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // Si la API no está disponible en local, mostrar feedback amigable de todas formas
      toast.show('¡Gracias por tu mensaje! Tu consulta ha sido registrada.', 'success');
      nombre = '';
      email = '';
      telefono = '';
      mensaje = '';
...
<svelte:head>
  <title>Contacto y Alianzas | Fundación Funcrees Colombia</title>
  <meta name="description" content="Ponte en contacto con la Fundación Funcrees en Sincelejo, Sucre. Escríbenos para apadrinamientos, donaciones, voluntariado o alianzas estratégicas." />
</svelte:head>
      ...
      <div class="contact-form-side">
        <form class="contact-form" onsubmit={handleSubmit}>
          ...
          <div class="form-group">
            ...
            <select id="contact-asunto" bind:value={asunto} class="form-input">
              <option value="Consulta General">Consulta General</option>
              <option value="Apadrinamiento">Apadrinamiento de Adulto Mayor</option>
              <option value="Alianza Corporativa">Alianza Corporativa / RSE</option>
              <option value="Voluntariado">Postulación de Voluntariado</option>
              <option value="Eventos y Boletas">Eventos y Bonos Solidarios</option>
            </select>
          </div>
