<script lang="ts">
        ...
        import { calcularImpacto, equivalenciaPrincipal } from '$lib/utils/impacto';
        ...
        const IMPACTO_MIN = 8000;
        const IMPACTO_MAX = 1000000;
        const IMPACTO_STEP = 2000;
        /** Monto del slider, independiente del checkout (solo informativo). */
        let montoImpacto = $state(50000);
        const impactoActual = $derived(calcularImpacto(montoImpacto));
        const impactoFrase = $derived(equivalenciaPrincipal(montoImpacto));

        function abrirDonacionConImpacto(): void {
                selectedPreset = 0;
                customAmount = formatMoneyNumber(montoImpacto);
                openModal('donacion');
        }
        // ─────────────────────────────────────────────────────────────────
...
<section id="donaciones-aliados" class="view-section active">
        <div class="section">
                ...
                <!-- Calculadora de impacto: ¿qué financia tu aporte? -->
                <div class="impacto-card" aria-labelledby="impacto-titulo">
                        <div class="impacto-header">
                                <span class="section-subtitle">Transparencia desde el primer paso</span>
                                <h2 id="impacto-titulo">¿Qué financia tu aporte?</h2>
                                <p>
                                        Mueve el selector y descubre el impacto concreto de tu donación en la
                                        vida de nuestros adultos mayores.
                                </p>
                        </div>

                        <div class="impacto-cuerpo">
                                <div class="impacto-monto" aria-live="polite">
                                        <span class="impacto-monto-valor">${formatMoneyNumber(montoImpacto)} COP</span>
                                        <span class="impacto-monto-frase">{impactoFrase || 'desliza para ver tu impacto'}</span>
                                </div>

                                <input
                                        type="range"
                                        class="impacto-slider"
                                        min={IMPACTO_MIN}
                                        max={IMPACTO_MAX}
                                        step={IMPACTO_STEP}
                                        bind:value={montoImpacto}
                                        aria-label="Monto de la donación en pesos colombianos"
                                />
                                <div class="impacto-extremos" aria-hidden="true">
                                        <span>$8.000</span>
                                        <span>$1.000.000</span>
                                </div>

                                <div class="impacto-equivalencias">
                                        <div class="impacto-chip" class:impacto-chip--cero={impactoActual.almuerzos === 0}>
                                                <i class="fa-solid fa-utensils" aria-hidden="true"></i>
                                                <strong>{impactoActual.almuerzos}</strong>
                                                <span>{impactoActual.almuerzos === 1 ? 'almuerzo' : 'almuerzos'}</span>
                                        </div>
                                        <div class="impacto-chip" class:impacto-chip--cero={impactoActual.sesionesTerapia === 0}>
                                                <i class="fa-solid fa-person-walking" aria-hidden="true"></i>
                                                <strong>{impactoActual.sesionesTerapia}</strong>
                                                <span>{impactoActual.sesionesTerapia === 1 ? 'sesión de terapia' : 'sesiones de terapia'}</span>
                                        </div>
                                        <div class="impacto-chip" class:impacto-chip--cero={impactoActual.kitsMedicamentos === 0}>
                                                <i class="fa-solid fa-pills" aria-hidden="true"></i>
                                                <strong>{impactoActual.kitsMedicamentos}</strong>
                                                <span>{impactoActual.kitsMedicamentos === 1 ? 'kit mensual de medicamentos' : 'kits mensuales de medicamentos'}</span>
                                        </div>
                                </div>

                                <button type="button" class="btn btn-primary impacto-donar" onclick={abrirDonacionConImpacto}>
                                        <i class="fa-solid fa-hand-holding-heart" aria-hidden="true"></i>
                                        Donar ${formatMoneyNumber(montoImpacto)} COP
                                </button>
                                <p class="impacto-nota">
                                        Cifras estimadas de operación 2026: almuerzo $8.000 · terapia $25.000 ·
                                        kit mensual de medicamentos $40.000.
                                </p>
                        </div>
                </div>
