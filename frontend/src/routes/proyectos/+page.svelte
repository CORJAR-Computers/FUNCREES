<script>
  import { PROYECTOS, getTagClass } from '$lib/data/proyectos.js';
  import { toast } from '$lib/stores/toast.svelte.js';

  let searchQuery = $state('');
  let selectedTag = $state('Todos');
  /** @type {import('$lib/data/proyectos.js').Proyecto | null} */
  ...
  /**
   * @param {import('$lib/data/proyectos.js').Proyecto} project
   */
...
<svelte:head>
  <title>Nuestros Proyectos | Fundación Funcrees Colombia</title>
  <meta name="description" content="Explora los proyectos sociales de Funcrees Colombia: atención al adulto mayor, empoderamiento juvenil, soberanía alimentaria y bienestar comunitario en Sucre." />
</svelte:head>
...
<!-- Modal de Detalle del Proyecto accesible -->
{#if selectedProject}
  ...
  >
    ...
    >
      <button class="modal-close" onclick={closeProjectModal} aria-label="Cerrar modal">✕</button>
      <div style="max-height: 80vh; overflow-y: auto; padding-right: 0.5rem;">
        ...
        <h2 style="color: var(--secondary); margin-bottom: 0.75rem;">{selectedProject.title}</h2>
        ...
        <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
          <a
            ...
            onclick={() => {
              closeProjectModal();
              toast.show(`Gracias por tu interés en apoyar: ${selectedProject?.title}`, 'success');
            }}
          >
